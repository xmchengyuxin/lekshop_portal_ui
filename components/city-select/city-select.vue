<template>
	...
	<view class="city-select">
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="city-select-main" id="city-select-main" :scroll-into-view="toView">
			<!-- 预留搜索-->
			<view class="flex city-serach" v-if="isSearch" :style="{ 'padding-top': top + 'px' }">
				<view class="flex flex-1">
					<input @input="keyInput" :placeholder="placeholder" class="city-serach-input w100 " />
				</view>
				<view @click="back()" class="flex f-s-0 f-a-c f-j-c padding-lr10">取消</view>
				<view class="flex f-s-0 w-15"></view>
				<!-- #ifdef MP-WEIXIN -->
				<view class="flex f-s-0" style="width: 200upx;"></view>
				<!-- #endif -->
			</view>
			<!-- 当前定位城市 -->
			<view class="hot-title" v-if="activeCity && !serachCity">当前定位城市</view>
			<view class="hot-city flex f-a-c f-j-s" v-if="activeCity && !serachCity">
				<view class="hot-item" @click="cityTrigger(activeCity)">{{ activeCity[formatName] }}</view>
				<view @click="refreshLocation()" class="flex f-a-c" style="margin-bottom: 6px;">
					<view class="flex f-a-c f-j-c van-icon van-icon-replay f18-size margin-r4"></view>
					<text>重新定位</text>
				</view>
			</view>
			<!-- 热门城市 -->
