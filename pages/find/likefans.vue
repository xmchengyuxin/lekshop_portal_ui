<template>
	<view class="contain ">
		...
		<view v-for="(item,index) in list" class="bg-color-w b-radius-5 padding-10 flex margin-b12">
			<view class="flex f-s-0 w-50 h-50 b-radius bg-img margin-r12" :style="item.viewMemberHeadImg | bgimg(300)+''"></view>
			<view class="flex flex-1 f-c f-j-c">
				<view class="flex">{{item.viewMemberName}}</view>
				<view @click="go('/pages/find/user?id='+item.viewMemberId)" class="flex f-a-c margin-t2">
			...
			<!-- <view class="flex f-a-c f-j-c f-s-0">
				<view @click="del(index)"  class="flex f-a-c f-j-c b-radius-5 bg-color-e padding-lr10 h-30">
					...
					<text class="f12-size">{{i18n['查看']}}</text>
				</view>
			</view> -->
			...
			del(index) {
				$.showModal({
					...
					success() {
						$.ajax({
							url: API.likeUserApi,
							data: {walkMemberId: self.list[index]['walkMemberId']},
							method: 'POST',
							success(res) {
								self.i18n['操作成功'];
