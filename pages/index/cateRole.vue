<template>
	<view class="contain">
		<view class="flex f-s-0 h-44 bg-color-w b-bottom box-c" :style="{ 'padding-top': top + 'px' }">
			...
			<view @click="go('/pages/search/roleList',2)" class="flex f-a-c flex-1">
			...
			<view  class="flex f-a-c f-j-c padding-lr10  f20-size"></view>
		...
		<view class="flex">
			...
			<scroll-view v-if="cateList.length > 0" scroll-y="true" class="flex f-c f-s-0 flex-1 padding-10" :style="{'height': height+'px'}">
				<view v-for="item in cateList[active]['children']" class="bg-color-w b-radius-10  wrap-sub-cate padding-10">
					<view  @click="go('/pages/search/roleList?cateTid='+item.id+'&cateName='+item.name,2)" class="flex f-a-c f-j-s">
						<text class="t-color-5 f12-size">{{item.name}}</text>
						<text class="flex f-a-c f-j-c van-icon van-icon-arrow t-color-9 f16-size"></text>
					</view>
					<view class="flex f-w">
						<view @click="go('/pages/search/roleList?cateId='+child.id+'&cateName='+child.name,2)" v-for="(child,idx) in item.children" class="flex f-s-0 sub-item f-c f-a-c f-j-c margin-t20">
...
<script>
	...
	export default {
		...
		methods: {
			getCate() {
				$.ajax({
					url: 'common/point/goodsCate/getList',
					data: {},
					method: 'GET',
					success(res) {
