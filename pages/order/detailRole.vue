		...
		<view class="padding-12">
			...
			<view class="bg-color-w b-radius-5 padding-12 margin-b12">
				<view class="margin-b10" v-for="(item,index) in goodsList">
					<view @click="go('/pages/shops/roleDetail?id='+item.goodsId)" class="flex">
						...
						<view class="flex flex-1 f-c margin-r8">
							...
							<view class="flex f-a-c margin-t6">
								<view class="text-price-role t-color-p f12-size">{{item.buyPoint}}</view>
								<view class=" t-color-p f12-size padding-lr2">+</view>
								<view class="text-price t-color-p f12-size">{{item.buyPrice}}</view>
							</view>
						</view>
						<view class="flex f-s-0 f-c">
							<view class="text-price f12-size">{{item.buyOriPrice}}</view>
				...
				<view class="flex f-j-e f12-size t-color-y f-w-500 margin-t6">
					<text>{{i18n['实付款']}}：</text>
					<text class="text-price-role">{{order.buyTotalPoint}}</text>
					<text class="">+</text>
					<text class="text-price">{{order.buyTotalPrice}}</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			showWuliu() {
				this.$refs.wuliuinfo.showRole(this.order.id);
			},
			sureOrder() {
				$.ajax({
			...
			getDetail() {
				$.ajax({
					...
					success(res) {
						...
						if(info == ''){return}
						self.goodsList = info ? [info] : [];
						self.order = info ? info : '';
						self.now = res.now;
