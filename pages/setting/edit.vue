	...
	<view class="contain">
		<view class="contain-page top-0">
			...
			<view class="padding-15 flex f-j-s b-bottom">
				<view class="f-w-b">{{i18n['手机号']}}</view>
				<view class="flex flex-1">
					<input v-model="phone" type="number" disabled="true" class="w100 h100 f-w-500 right" value="" :placeholder="i18n['请输入']" />
				</view>
			</view>
			<view class="padding-15 flex f-j-s b-bottom">
			...
			<view class="padding-15 flex f-j-s b-bottom">
				...
				<view class="flex flex-1 f-j-e">
					<view @click="sex='1'" class="flex f-a-c f-j-c margin-r20">
						...
						<text class="t-color-b f-w-500">{{i18n['男']}}</text>
					</view>
					<view @click="sex='2'" class="flex f-a-c f-j-c">
						...
						<text class="t-color-b f-w-500">{{i18n['女']}}</text>
					</view>
				</view>
			</view>
		...
		<view class="warp-tabbar flex f-a-c padding-lr12" :style="{'padding-bottom': isIphonex ? '34px' : ''}">
			<view @click="submit" class="flex flex-1 f-a-c f-j-c b-radius-30 bg-color-linear-y t-color-w h-36">{{i18n['保存']}}</view>
		</view>
	</view>
</template>
<style scoped>
